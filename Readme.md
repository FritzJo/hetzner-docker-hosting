...
* Letsencrypt SSL Proxy for all Docker services ([nginx-proxy](https://github.com/nginx-proxy/nginx-proxy))
...
* [OpenTofu](https://opentofu.org/docs/intro/install/) (or [Terraform](https://www.terraform.io/downloads.html))
...
For specific details on how to use Portainer, check the [official documentation](https://docs.portainer.io/)
### Automation
...
| Backup | Automated backups to GCP Storage run daily at 1:00 AM. For more information (like how to change the repo password) check the official [documentation](https://restic.readthedocs.io/en/latest/070_encryption.html)|
...
If you see an error like `"Could not find or access '../custom/secrets/gcp-secret.json'`, make sure the file exists at `custom/secrets/gcp-secret.json`. Copy your secrets directory into the `custom/` folder.

### 2. Are ARM VMs also supported?
Yes! Simply change the `hcloud_server_type` in `custom/terraform.tfvars` to the desired ARM server type (e.g., `cax11`) and adapt your Dockerfiles accordingly.